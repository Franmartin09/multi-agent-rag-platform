...
# ---------------------------------------------------------------------------
DOCS_DIR = Path("./docs")
OUTPUT_DIR = Path("./db/preprocessed")
...
class DiagramComponent(BaseModel):
    name: str = Field(description="Name of the component appearing in the diagram.")
    description: str = Field(description="Technical description of the component and its role.")

class TechnicalDetail(BaseModel):
    detail: str = Field(description="Relevant technical information extracted from the diagram.")

class WarningItem(BaseModel):
    warning: str = Field(description="Warning, note or constraint shown in the diagram.")

class DiagramDescription(BaseModel):
    purpose: str = Field(description="Purpose of the diagram.")
    summary: str = Field(description="Overall explanation of what the diagram teaches.")
    components: List[DiagramComponent] = Field(default_factory=list, description="Components, tools, materials or parts appearing in the diagram.")
    procedure: List[str] = Field(default_factory=list, description="Ordered list of steps represented by the diagram.")
    technical_details: List[TechnicalDetail] = Field(default_factory=list, description="Additional relevant technical details.")
    warnings: List[WarningItem] = Field(default_factory=list, description="Warnings, precautions or constraints.")
...
# Function to embed and insert text in Postgres
def insert_chunk_into_db(conn, doc_name: str, page_idx: int, chunk_type: str, content: str, metadata: dict):
    ...
    try:
        ...
        time.sleep(7)  # Short pause to avoid overloading the API
        # Insert into Postgres
        with conn.cursor() as cur:
            cur.execute("""
                INSERT INTO document_chunks (document_name, page_number, chunk_type, content, embedding, metadata)
            ...
            """, (
                ...
                chunk_type,
                content,
                vector_embedding, # psycopg2 automatically adapts it to a pgvector-compatible array
                Json(metadata)    # Convert the Python dict to JSONB
            ))
        conn.commit()
    except Exception as e:
        print(f"Error inserting chunk into database (Page {page_idx}, {chunk_type}): {e}")
        conn.rollback()
    ...
    for t_idx, table in enumerate(tables, start=1):
        ...
        if extracted_table:
            ...
            tables_meta.append({
                ...
                "bbox": table.bbox,
                "content": table_to_markdown(extracted_table)
            })
...
def process_pdf(pdf_path: Path, db_conn):
    ...
    with pdfplumber.open(pdf_path) as pdf_plumber:
        for page_idx, page in enumerate(pdf_plumber.pages, start=1):
            ...
            # Extract text excluding tables and figures
            ...
            # Integrate the tables in Markdown format within the same text
            combined_text = free_text
            if tables_meta:
                tables_markdown_list = []
                for t in tables_meta:
                    tables_markdown_list.append(f"\n\n### Tabla ({t['table_id']})\n{t['content']}")
                
                if combined_text:
                    combined_text += "".join(tables_markdown_list)
                else:
                    combined_text = "".join(tables_markdown_list).strip()

            # Save unified JSON chunk only if the page contains content
            if combined_text or tables_meta or figures_meta:
                page_chunk = {
                    "document": pdf_path.name,
                    "page": page_idx,
                    "free_text": combined_text,  # Contiene tanto el texto libre como las tablas en Markdown
                    "tables": tables_meta,
                ...
                # Insertion into PostgreSQL (Vector DB)
                # Insert Unified Text (Free Text + Markdown Tables together)
                if combined_text:
                    insert_chunk_into_db(db_conn, pdf_path.name, page_idx, 'text', combined_text, {"source": "free_text_and_tables"})
                
                # B. Insert Unified Figure Content
                for fig in figures_meta:
                    desc = fig['gemini_description']
                    
                    fig_content = f"Figure from page {page_idx}.\n"
                    fig_content += f"Purpose: {desc.get('purpose', '')}\n"
                    fig_content += f"Summary: {desc.get('summary', '')}\n"
                    
                    components = ", ".join([c.get('name', '') for c in desc.get('components', [])])
                    if components: 
                        fig_content += f"Components: {components}\n"
                        
                    procedure = " ".join(desc.get('procedure', []))
                    if procedure: 
                        fig_content += f"Procedure: {procedure}\n"
                        
                    warnings = " ".join([w.get('warning', '') for w in desc.get('warnings', [])])
                    if warnings: 
                        fig_content += f"Warnings: {warnings}\n"

                    f_meta = {"png_file": fig['png_file'], "bbox": fig['bbox'], "gemini_json": desc}
                    insert_chunk_into_db(db_conn, pdf_path.name, page_idx, 'figure', fig_content, f_meta)

                print(f"  [Page {page_idx}] -> Processed and inserted into the database. (Text/Tables combined length: {len(combined_text)} | Tables: {len(tables_meta)} | Figures: {len(figures_meta)})")
...
def main():
    ...
    conn.close()
    print(f"\n Processing completed successfully. Check '{OUTPUT_DIR.resolve()}' and your PostgreSQL Database.")
